...
public:
    ...
    std::vector<Envelope> roadEnvelopes;
    ...
    Envelope createRoadEnvelope(const Segment& segment, double width);

    std::vector<Segment>& getConnectedSegments(const Point& point);
